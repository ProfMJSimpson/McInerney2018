...
% Author: Sean McInerney; Last Update: 17/05/18
...
if x < L1
    % Skin layer solution (Refer to Equation 39 in supplementary material)
    TBar = cosh(xi1*(x-L1))/(s*cosh(xi1*L1)) - ...
        (D2*xi2*sinh(xi1*x)*tanh(xi2*(L2-L1))) / ...
        (s*cosh(xi1*L1)^2*(D2*xi2*tanh(xi1*L1)*tanh(xi2*(L2-L1))+D1*xi1));
else
    % Fat layer solution (Refer to Equation 40 in supplementary material)
